...
const CrearClienteModal: React.FC<Props> = ({ onClose, onCrear }) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!/^\d+$/.test(formData.Celular)) {
      return Swal.fire({ icon: 'error', title: 'Celular inválido', text: 'Solo dígitos.', confirmButtonColor: '#e83e8c' });
    }
    if (!/^\d+$/.test(formData.Numerodocumento)) {
      return Swal.fire({ icon: 'error', title: 'Documento inválido', text: 'Solo dígitos.', confirmButtonColor: '#e83e8c' });
    }
