...
const FinalizarCompraModal: React.FC<Props> = ({ visible, onClose, onEnviar }) => {
  ...
  // Inicializar valor inicial al 50% cuando cambia el total
  useEffect(() => {
    const valorDefault = total * 0.5;
    setValorInicial(valorDefault);
    setValorInicialInput(`$${valorDefault.toLocaleString('es-CO')}`);
  }, [total]);
  ...
  const handleSubmit = async () => {
    ...
    if (!file || !filePreview) {
      Swal.fire("Comprobante requerido", "Por favor sube el comprobante de pago.", "warning");
      return;
    }
    ...
    try {
      ...
      const pedidoConDetalles = {
        ...
        MetodoPago: metodo,
        FechaPedido: new Date(fechaServidor + "T00:00:00").toISOString(),
        ...
        IdEstado: valorInicial >= total ? 1007 : 1, // 1007 si paga todo, 1 si paga inicial
    ...
    <div className="overlay-modal">
      <div className="modal-finalizar scrollable">
        ...
        <div className="formulario-tres-columnas">
          ...
          <div className="input-icono" data-label="Valor inicial (50% mín)">
  ...
  onChange={(e) => {
    ...
    setValorInicial(numero);
    setValorInicialInput(`$${numero.toLocaleString('es-CO')}`);
  }}
  onBlur={(e) => {
    const valor = valorInicial;
    const minimo = total * 0.5;

    if (valor === 0 || e.target.value === '' || e.target.value === '$') {
      Swal.fire(
        "Campo requerido",
        `Debes ingresar el pago inicial (mínimo $${minimo.toLocaleString('es-CO')} COP)`,
        "warning"
      );
      setValorInicial(minimo);
      setValorInicialInput(`$${minimo.toLocaleString('es-CO')}`);
    } else if (valor < minimo) {
      Swal.fire(
        "Valor bajo",
        `El pago inicial mínimo es $${minimo.toLocaleString('es-CO')} COP (50% del total)`,
        "warning"
      );
      setValorInicial(minimo);
      setValorInicialInput(`$${minimo.toLocaleString('es-CO')}`);
      ...
      Swal.fire(
        "Valor alto",
        `El pago inicial no puede superar el total ($${total.toLocaleString('es-CO')} COP)`,
        "warning"
      );
      setValorInicial(total);
      setValorInicialInput(`$${total.toLocaleString('es-CO')}`);
    }
  }}
  placeholder="$0"
/>
        ...
        <div className="resumen-pago">
          ...
          <p>
            <FaCreditCard /> <strong>Pago inicial:</strong> ${pagoInicial.toLocaleString()} COP
          </p>
          <p>
